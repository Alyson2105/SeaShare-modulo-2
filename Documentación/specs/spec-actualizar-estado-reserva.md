...
4. **Scenario**: Aviso de cancelación o inasistencia, para aplicar penalidades y reembolsos
    ...
    - **Then** el sistema le avisa a Módulo 3 el estado "Cancelado" junto con su sub-estado (determinado internamente en Módulo 2 según las reglas de "Solicitar cancelación" o "Marcar inasistencia") para que Módulo 3 aplique su matriz de liquidación y reembolsos
...
- **FR-002**: El sistema DEBE seguir esta lista oficial de estados de la reserva:
    ...
    - 🔶 [PENDIENTE DE CONFIRMAR — Sub-estados de Cancelación]: Solo aplican cuando el estado principal es `Cancelado`. Los valores permitidos son: `Flexible`, `Moderado`, `Tardío`, `Por Anfitrión`, `Por Inasistencia`. Todos ellos se determinan internamente en Módulo 2 según las reglas de anticipación temporal o inasistencia (dentro de "Solicitar cancelación" para `Flexible`, `Moderado`, `Tardío` y `Por Anfitrión`; y dentro de "Marcar inasistencia" para `Por Inasistencia`). Este caso de uso recibe dicho sub-estado como parámetro de entrada, lo persiste en la reserva y delega su notificación a Módulo 3 a través de "Recibir estado de reserva" para que Módulo 3 ejecute la liquidación de fondos. [FIN PENDIENTE]
...
- **FR-010**: **REGLA DE NEGOCIO ESTRICTA (Sin cálculo financiero):** El sistema **NO DEBE calcular montos de reembolso, porcentajes de penalidad, comisiones de la plataforma, costos de seguros ni hacer transferencias de dinero**. La responsabilidad de Módulo 2 se limita a manejar el tiempo, gobernar la máquina de estados, clasificar los sub-estados por reglas de anticipación temporal o inasistencia y notificar el cambio a Módulo 3; todo cálculo de dinero, valoración económica y liquidación de fondos es responsabilidad exclusiva de Módulo 3.
